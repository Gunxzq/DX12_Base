...
/// ANI 母版骨架部件（部件名 + A/B 层级，矩阵由 ANI 首帧 HOD 提供）
...
public:
    ...
    /// 获取母版骨架（部件名 + A/B 层级；矩阵由 ANI 首帧 HOD 提供）
...
private:
    ...
    bool ParseTail(const uint8_t *data, size_t size, size_t tailOffset, size_t tailLimit, bool isLastGroup,
                   ANITail &out) const;
