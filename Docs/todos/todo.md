...
12. LOD的阈值需要动态切换能力
13. ✅ 调度系统已实现依赖解析（`DependsOn`/`AddDependency`，TaskGraph 支持拓扑排序 + 环检测）
14. ✅ 构建器已实现并行化 + 临时结构（PendingBatch + FrameSync 统一上传）
15. 我们应该限制鼠标移动到窗口外，起码不能影响相机的旋转
16. 确保系统的模块可以正常的释放资源
17. 统一坐标系和矩阵存储模式
18. ✅ ConvertToGPUConstants() 已正确使用 `baseColorTextureId`/`normalTextureId` 等字段，不再是硬编码 0
19. 实体销毁时需要释放持久化缓存，构建器存在多线程安全性问题
20. 系统是左手坐标系
21. 更安全的动静分批，静态组件，八叉树空间划分，预计算遮挡
22. 一定要开启调试层的能力
23. 当前阶段使用static_assert来检查16字节对齐
24. imgui是自治的，假定资源是COMMON的
25. 事件系统的枚举需要处理
26. 命令列表提交之前存在资源状态校验
27. 必须在帧同步点更新ECS
28. 需要一个时机处理各处硬编码的配置
29. 反射探针的优先级更新，后续需要改为可配置的
30. 暂时并没有单位系统，基于约定的，1.0f = 1米，后续需要统一此内容
31. ✅ DSV/RTV 池已存在（`RenderTargetPool` 用于反射探针，`DepthStencilPool` 用于深度缓冲）
32. 展示的镜面效果需要在后续中进行优化处理
33. 区块应该从正确的地方加载材质数据
34. 描述符堆需要考虑到场景的切换
