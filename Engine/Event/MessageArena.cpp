...
void MessageArena::AllocateRingMemory() {
    // 分配环形缓冲区的四个定长数组
    m_ring.typeBuffer = static_cast<EventTypeHash *>(
        ArenaAlignedMalloc(RING_BUFFER_SIZE * sizeof(EventTypeHash), ALIGNMENT_CACHE_LINE));
    m_ring.senderBuffer =
...
MessageArena::MessageArena(uint32_t capacity)
    : m_globalIndexCounter(0), m_currentFrameStartIndex(0), m_currentFrameMessageCount(0),
      m_currentFrameTypeBuffer(nullptr), m_currentFrameSenderBuffer(nullptr), m_currentFrameTimeBuffer(nullptr),
      m_currentFramePayloadBuffer(nullptr), m_tlsContextCount(0), m_overflowCount(0), m_lastFlushedIndex(0) {
    memset(m_tlsContexts, 0, sizeof(m_tlsContexts));
    AllocateRingMemory();
}

MessageArena::~MessageArena() { FreeRingMemory(); }
